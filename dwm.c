...
static const char autostartblocksh[] = "scripts/startup_blocking";
static const char autostartsh[] = "scripts/startup";
...
drawbar(Monitor *m)
{
    ...
    if (m == selmon || 1) { /* status is only drawn on selected monitor */
        // except we're drawing it on all monitors so yes
...
setup(void)
{
    ...
    lrpad = drw->font->h;
    bh = user_bh ? user_bh : drw->font->h + 2;
    updategeom();
    /* init atoms */
